#!/usr/bin/env python3
"""把天工双夹爪采集数据转换为 LeRobot 数据集。

默认用 13 条轨迹训练、1 条轨迹留出：`0000` 到 `0012` 写入训练集，
`0013` 不写入。状态和动作从原始双手数据中只取每侧手部第 1 维作为夹爪自由度，
形成左臂7 + 左夹爪1 + 右臂7 + 右夹爪1 的 16 维向量。
"""
...
def _create_dataset(repo_id: str, *, root: Path, fps: int, overwrite: bool) -> LeRobotDataset:
    ...
    return LeRobotDataset.create(
        repo_id=repo_id,
        root=output_path,
        robot_type="tienkung_dual_grippers",
        fps=fps,
        features={
            "base_image": {
                "dtype": "image",
...
def convert(
    ...
    *,
    ...
    if push_to_hub:
        dataset.push_to_hub(
            tags=["tienkung", "dual-grippers", "openpi"],
            private=True,
            push_videos=True,
            license="apache-2.0",
        )
