...
- `pi0.5` 风格的更高层语义泛化，以及受 knowledge-insulation 启发的实验。
...
- [x] 保留上游 OpenPI 致谢、许可证文件和 README 引用。
- [x] 保持项目为仅代码镜像，不包含模型权重、数据集、虚拟环境或训练日志。
- [x] 添加公开项目 README，说明非官方范围。
- [ ] 在每次项目变更后保持上游 OpenPI baseline 可复现。
- [ ] 为新的模型、数据和 policy-server 路径添加项目 smoke tests。
...
- [ ] 探索用于语义泛化的多任务或多场景混合。
- [ ] 尝试受控冻结、adapter、数据加权或受 knowledge-insulation 启发的训练配方。
...
- [ ] 为 success、failure、evaluation episodes 和 human intervention 定义通用 episode metadata。
- [ ] 为 `advantage_indicator`、`use_advantage` 和 `is_human_intervention` 生成 sidecar labels。
- [ ] 添加用于离线 advantage labeling 的 value-proxy 或 progress-proxy 工具。
- [ ] 在不修改原始数据集的情况下，将 RECAP sidecar fields 合并进 dataloader。
- [ ] 添加 advantage-conditioned prompt、token 或 embedding 路径。
- [ ] 添加用于 1-2 step smoke tests 的小型 debug config。
- [ ] 对比标准 SFT 和 RECAP 风格 fine-tuning。

### 6. MEM / 记忆

- [ ] 调研公开 MEM 相关材料和可复现实现线索。
- [ ] 为 recent history、task state、failures 和 recovery hints 设计可选 memory/context fields。
- [ ] 保持 memory inputs 可选，使现有 OpenPI policies 仍然兼容。
