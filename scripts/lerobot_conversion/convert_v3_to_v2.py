...
try:
    ...
    if exc.name is not None and (exc.name == "lerobot" or exc.name.startswith("lerobot.")):
        # OpenPI 当前主环境固定在旧版 LeRobot，这里只补齐转换脚本实际需要的
        # v3/v2.1 元数据工具，避免为了转换数据安装完整新版 LeRobot 和 torch 栈。
        ...
        def load_info(local_dir: Path) -> dict:
            with open(local_dir / "meta" / "info.json", encoding="utf-8") as f:
                return json.load(f)
        ...
        def write_info(info: dict, local_dir: Path) -> None:
            ...
            with open(out_path, "w", encoding="utf-8") as f:
                json.dump(_to_serializable(info), f, ensure_ascii=False, indent=2)

        def load_tasks(local_dir: Path) -> list[dict[str, Any]]:
            table = pq.read_table(local_dir / "meta" / "tasks.parquet")
            rows = []
            for row in table.to_pylist():
                task = row.get("task") or row.get("__index_level_0__")
                rows.append({"task_index": int(row["task_index"]), "task": task})
